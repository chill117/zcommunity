	...
	<table width="100%" border="0" cellspacing="0" cellpadding="4">
					...
					<table width="100%" cellspacing="0" cellpadding="0" border="0">
						...
						<tr class="needsPadding">
							<td align="left" width="95%">
								' . $txt['b40'] . ' ' . $article['poster']['link'] . ' ' . $txt['b3034'] . ' ' . $article['time'] . '
							</td>
		...
		if (!empty($article['show_comments']))
		{
			...
			if (!empty($article['comments']) && !empty($article['page_index']) && !empty($article['show_page_index']) && !empty($context['viewing_single_article']))
				echo $article['page_index'] . '&nbsp;&nbsp;'. $article['show_all_comments_link'];
			// otherwise show a link to single article page... first comment and all comments (if some comments were not shown here)
			elseif (!empty($article['show_all_comments_link']))
				echo $article['show_all_comments_link'];
				
			echo '
							</td>
							<td align="right">
								' . $article['num_comments'] . '&nbsp;' . ($article['num_comments'] == 1 ? $txt['b15'] : $txt['b15a']) . ($article['can_reply'] ? '&nbsp;<span style="font-size:14px;color:#C6C6C6;">|</span>&nbsp;'. $article['new_comment'] : '') .'
							</td>
						</tr>';
		}
		...
		if (!empty($context['zc']['previous_link']) && !empty($context['zc']['next_link']))
			echo '
						<tr class="needsPadding"><td align="center">', $context['zc']['previous_link'], '&nbsp;&nbsp;', $context['zc']['next_link'], '</td></tr>';
					
		// extra stuff in between article and comments?
		if (!empty($article['extra']['between_article_and_comments']))
			echo '
						<tr class="needsPadding"><td align="center">', implode('&nbsp;&nbsp;', $article['extra']['between_article_and_comments']), '</td></tr>';
...
function template_showComment($comment, $c = null)
{
	...
	echo '
				...
				<table border="0" width="100%" cellspacing="1" cellpadding="4" id="c'. $comment['id'] .'" class="bordercolor">
					<tr class="windowbg', $c%2 ? '' : '2', empty($comment['is_approved']) && !empty($comment['approve_link']) ? '-u' : '', '">
						<td width="100%">
